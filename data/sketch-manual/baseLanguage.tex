...
\paragraph{Modeling floating point}
Analyzing floating point values is very expensive for the synthesizer,
so as of version 1.7.5, the solver provides four different representations that the analyzer
can use to reason about either \C{float} or \C{double}. The flag
\C{--fe-fpencoding} controls which representation to use.

\flagdoc{fe-fpencoding}{ This flag controls which of three possible encodings are used for floating point values.
\C{AS_BIT} encodes floating point values using a single bit; addition and
subtraction are replaced with \C{xor}, and multiplication is replaced with
\C{and}. Division and comparisons are not supported in this representation, nor
are casts to and from integers. \C{AS_FFIELD} will encode floating points using
a finite field of integers mod 7. This representation does support division, but
not comparisons or casts. Finally, \C{AS_FIXPOINT} represents floats as fixed
point values; this representation supports all the operations, but it is expensive.
Finally, the \C{TO_BACKEND} representation uses the floating point support in
the backend solver. It is the most expressive representation, and is set by
default when you include the \C{math.skh} library. Its cost is very problem
specific; for some problems it can be very efficient, but it can also exhibit
poor scalability.}
...
\item The iteration variable of a cannonical \C{for} loop is final within the body of the loop. A cannonical \C{for} loop is a loop of the form \C{for(int i=a; i<b; ++i)}, where the iteration variable is not modified inside the body of the loop.
...
The variables \C{t1} and \C{t2} are considered to have the same value, and cannot be distinguished from each other. So the following assertion will pass:
\begin{lstlisting}
assert t1 == t2;
\end{lstlisting}
...
For recursive ADTs, \Sk{} will automatically generate a recursive comparison function to test equality, and will be subject to the inlining bounds given through the command-line flag \C{bnd-inline-amnt}.
In the context of ADTs, the constant \C{null} is just a special value that can only be compared against. ADT variables are all initialized to \C{null} if they are not explicitly initialized, so you can use comparisons such as
\begin{lstlisting}
if( t1 == null){

}
\end{lstlisting}
to check if a variable has been initialized, but any other operation that you try to do on a \C{null} value (reading a field, explicit casting or switching on it) will lead to an assertion failure.

\subsection{Struct inheritance}
In some cases, we want to make use of the ability of ADTs to define sub-types and to use switch to distinguish between cases, but we do not want to be restricted by the immutability requirement. In this case, we can use \C{struct} inheritance to achieve that goal. For example, we can write:
\begin{lstlisting}
struct Tree{
        int x;
}
...
We can use the above Branch, Leaf and Tree types the same way we would have if we had defined them using the \C{adt} syntax. The difference is that now, the \C{Leaf}s and \C{Branch}es will be mutable, so you can change their value after they have been created. There is a cost to this, though, in that the synthesizer will be able to handle immutable ADT values much more efficiently than these mutable structures. Also keep in mind because these are \C{struct}s, equality behaves just like with other structs.
...
and generator functions (\secref{generators}). A polymorphic function can be declared by including type parameters in its signature:
\begin{lstlisting}
...
When calling \C{forall}, \C{T} can be used in place of any data-type, including primitive types, structures, adts, and arrays thereof. The type \C{T}, however, cannot be instantiated to a function type.
...
Starting in Sketch 1.7.5, the language also supports polymorphic structures
...
For example, the Array library (\secref{lib:array}) defines an  \C{SArray}
structure as:
\begin{lstlisting}
struct SArray<T>{
    int n;
...
the type is instantiated as \C{int} for \C{ar1}, and as \C{SArray<int>}
in the case of \C{ar2}. Note that in the example we also define a
polymorphic function \C{len}, that works for \C{SArray<T>} for any
valid \C{T}.
...
in the \C{list} package (\secref{lib:list}).
\begin{lstlisting}
adt List<T>{
...
overall \C{List<T>} type. When invoking one of the constructors, we must
...
constants have type \C{int}, the constants \C{0} and \C{1} have
type \C{bit}, so if you have a function such as
\C{List<T> single<T>(T in); } and you call \C{single(0)}, the compiler will
infer that \C{T} is \C{bit}, so if you actually wanted a List<int>, then
you should either cast \C{0} to \C{(int)0} or assign it to a variable of
type \C{int}.
\end{enumerate}
...
Annotations are preserved throughout the compiler chain all the way to code generation. They can be particularly useful if you are writing your own custom code generator (see \secref{customcodegen}). In that case, you can just introduce annotations in your code, and your code generator can query the \C{Function} and  \C{StructDef} AST nodes to get their annotations (see the sample code generator provided for how to do this). There are, however, a handful of annotations that have special meaning to the compiler. Theey are listed below.


\begin{itemize}
\item \lstinline!@Native($code$)! overrides code generation for the annotated function (see \secref{nativecode}).
\item \lstinline!@NeedsInclude($inc$)!  tells the code generator that a particular function requires some specific header file to be included (see \secref{nativecode}).
